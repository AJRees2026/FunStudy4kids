...
export const MOODS: { key: MoodType; emoji: string; color: string; bgClass: string; textClass: string }[] = [
  { key: 'fantastic', emoji: '🌈', color: '#FFD166', bgClass: 'bg-[#FFD166]', textClass: 'text-[#9a6b00]' },
...
export default function MoodTracker({ childId, theme, isSpace }: Props) {
  ...
  return (
    <section className={`rounded-3xl p-4 ${theme.cardBg} border ${theme.cardBorder}`}>
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-4">
        {MOODS.map((m) => (
          <div key={m.key} className="flex items-center gap-1.5">
            <span className={`w-5 h-5 rounded-lg ${m.bgClass} flex items-center justify-center text-xs`}>{m.emoji}</span>
            <span className={`text-xs font-bold ${theme.textMuted}`}>{t(`mood${m.key.charAt(0).toUpperCase() + m.key.slice(1)}`)}</span>
          </div>
        ))}
      </div>
      ...
      {selectedDate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4" onClick={() => setSelectedDate(null)}>
          <div
            className={`rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-pop ${isSpace ? 'bg-slate-800 border border-slate-700' : 'bg-white'}`}
            ...
            <div className="grid grid-cols-5 gap-2 mb-4">
              {MOODS.map((m) => (
                ...
                >
                  ...
                  <span className={`text-[10px] font-bold ${selectedMood === m.key ? m.textClass : isSpace ? 'text-slate-400' : 'text-slate-500'}`}>
