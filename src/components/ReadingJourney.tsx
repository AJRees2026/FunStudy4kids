    ...
    <section className={`rounded-3xl p-4 ${theme.cardBg} border ${theme.cardBorder}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {!readOnly && (
          <button
            ...
            className={`shrink-0 rounded-xl px-3 py-2 font-display font-bold text-sm ${theme.actionButtonText} transition-500 hover:scale-105 active:scale-95 ${theme.actionButtonBg} flex items-center gap-1.5`}
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4" onClick={() => { stopCamera(); setShowModal(false) }}>
          <div
            className={`rounded-3xl p-6 max-w-md w-full shadow-2xl animate-pop max-h-[90vh] overflow-y-auto ${
              isSpace ? 'bg-white border border-slate-700' : 'bg-[#A8F0E0]'
            }`}
              ...
              <div>
                ...
                {coverMode === 'idle' && !displayCover && (
                  <div className="flex gap-2">
                    <button
                      onClick={startCamera}
                      className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-bold transition-all hover:scale-[1.02] active:scale-95 bg-[#0a173b] text-white`}
                    >
                      <Camera className="w-4 h-4" /> {t('photographCover')}
                    </button>
                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-bold transition-all hover:scale-[1.02] active:scale-95 bg-[#ff6700] text-white`}
                    >
      ...
      {toast && (
        <div className="fixed bottom-4 right-4 z-40 max-w-xs">
          <div className={`bg-gradient-to-r ${theme.buttonGradient} text-yellow-500 font-display font-bold px-5 py-3 rounded-2xl shadow-2xl animate-pop flex items-center gap-2`}>
